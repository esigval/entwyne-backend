...
export const validateAndRewriteNarratives = async (rawNarrative) => {
  ...
  const allowedTypes = narrativeBlocks.map(block => block.type);

  const narrativesToRewrite = [];
  const validatedNarratives = rawNarrative.map((block, index) => {
    if (!allowedTypes.includes(block.type)) {
      narrativesToRewrite.push({ index, block: { ...block, type: `${block.type}_Invalid` } });
      return { ...block, type: `${block.type}_Invalid` };
    }
    return block;
  });

  return { validatedNarratives, narrativesToRewrite, allowedTypes };
};
...
async function validateNarratives(narrativeData) {
  ...
  const { validatedNarratives, narrativesToRewrite, allowedTypes } = await validateAndRewriteNarratives(narrativeData.rawNarrative);

  if (narrativesToRewrite.length > 0) {
    const prompt = generateRewritePrompt(narrativesToRewrite, allowedTypes);
    ...
    return { ...narrativeData, rawNarrative: validatedNarratives };
  }
}

const reinsertNarratives = (originalNarratives, correctedNarratives) => {
  correctedNarratives.forEach(narrative => {
    originalNarratives[narrative.index] = {
      ...originalNarratives[narrative.index],
      ...narrative.block
    }; // Correct and replace the narrative at the original index
  });
