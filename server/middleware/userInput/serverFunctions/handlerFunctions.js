...
export function handleFunctionType(functionData, userId, twyneId) {
    let accumulatedArgs = "";
    accumulatedArgs += functionData.arguments;
    process.stdout.write(functionData.arguments);

    // Return the accumulatedArgs
    return accumulatedArgs;
}

export function handleToolCallDelta(delta, snapshot) {
    if (delta.type === 'function') {
        console.log('Tool Call Delta:', delta);
        accumulatedArgs += delta.function.arguments;   // Update the last known snapshot
    }
}
    ...
    try {
        ...
        if (accumulatedArgs) {
            ...
            if (toolCall.function.name === 'createRawNarrative') {
                const output = await processNarrativeBasedOnArgs(accumulatedArgs, userId, twyneId, threadId, toolCall.id, currentRunId);
                return output;
            } else if (toolCall.function.name === 'makeTwynes') {
                const output = await processCreateTwynes(accumulatedArgs, userId, storyId, toolCall.id, threadId, currentRunId);
            } else {
...
async function submitToolCallOutput(threadId, toolCallId, currentRunId, output) {
    const outputs = `We've produced ${output} prompts for you to review!`
